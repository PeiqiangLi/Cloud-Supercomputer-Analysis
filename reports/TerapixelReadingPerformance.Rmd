...
The relationship between these indexes and GPU operating temperature is shown in the figure below:
...
```
Due to each task corresponds to the rendering of a specific tile. 
...
```{r}
gpu_task_part %>%
  ggplot(aes(timestamp,mean_powet)) +
  geom_line(col="red")+
  xlab("timestamp") +
  ylab("gpuUtilPerc")
```
