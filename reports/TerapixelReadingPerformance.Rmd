...
```{r K-means,fig.width=8,out.width='80%'}
par(mfrow=c(1,2)) 
...
```{r}
bbb = gpu_task_part_average[2:(length(gpu_task_part_average$taskId)-1),] %>% 
ggplot(mapping=aes(x = duration, y = mean_temp)) +
  geom_smooth(formula = y ~ x, method='lm',color='red') +
  stat_cor(method = "spearman") +
  xlab("Mean task duration of this gpu") +
  ylab("temp when running this task") +
  geom_point(alpha = 0.6)

aaa = ggplot(gpu_task_part_average[-1,],mapping=aes(x = duration, y = mean_powet)) +
  geom_smooth(formula = y ~ x, method='lm',color='red') +
  stat_cor(method = "spearman") +
  xlab("Mean task duration of this gpu") +
  ylab("temp when running this task") +
...
ccc = ggplot(gpu_task_part_average[-1,],mapping=aes(x = duration, y = mean_gpuPerc)) +
  geom_smooth(formula = y ~ x, method='lm',color='red') +
  stat_cor(method = "spearman") +
  xlab("Mean task duration of this gpu") +
  ylab("temp when running this task") +
...
ddd = ggplot(gpu_task_part_average[-1],mapping=aes(x = duration, y = mean_memory)) +
  geom_smooth(formula = y ~ x, method='lm',color='red') +
  stat_cor(method = "spearman") +
  xlab("Mean task duration of this gpu") +
  ylab("temp when running this task") +
